...
export const createPlaybackSlice: StateCreator<PlayerState, [], [], any> = (set, get) => ({
  ...
  stopTrack: async () => {
    // Mark the stop intent before awaiting IPC/history writes. The backend can
    // report Stopped while those awaits are still in flight; end-of-track
    // recovery must not interpret that transient state as natural completion.
    const stopRequestedAt = Date.now();
    set(s => ({ playback: { ...s.playback, last_stop_time: stopRequestedAt } }));
    try {
  ...
  pollStatus: async () => {
    ...
    try {
      ...
      if (prevTrack && !newTrack && status.status === 'Stopped') {
        const now = Date.now();
        const lastSkipTime = currentPlayback.last_skip_time || 0;
        const sinceSkip = now - lastSkipTime;
        if (sinceSkip < 2000) {
          return;
        }

        // Rust normally emits `track-ended`, but a stream can fail while preparing
        // the next queued Tidal URL or the event can be missed during a renderer
        // restart. In that case the backend is already stopped while the frontend
        // still has the finished track and its queue. Recover from the end state
        // immediately instead of waiting five seconds and freezing at 100%.
        const userStoppedAfterTrackStarted = (currentPlayback.last_stop_time || 0) > lastSkipTime;
        if (
          currentPlayback.status === 'Playing' &&
          !currentPlayback.is_buffering &&
          !userStoppedAfterTrackStarted &&
          isAtNaturalEnd(currentTrack, currentPlayback.position_secs)
        ) {
          set(s => ({
            playback: {
              ...s.playback,
              ...
              current_track: null,
              is_buffering: false,
              backend_position_secs: status.position_secs,
